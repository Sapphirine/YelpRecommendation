...
def my_LeNet(input_x, input_y,
    ...
    norm_layer_0 = norm_layer(input_x = pooling_layer_0.output())
    #  conv layer1
    ...
    conv_w0 = [conv_layer_0.weight]
    conv_w1 = [conv_layer_1.weight]
    fc_w = [fc_layer_0.weight, fc_layer_1.weight]

    # loss
    with tf.name_scope("loss"):
        l2_loss = tf.reduce_sum([tf.norm(w) for w in fc_w])
        l2_loss += tf.reduce_sum([tf.norm(w, axis=[-2, -1]) for w in conv_w0])
        l2_loss += tf.reduce_sum([tf.norm(w, axis=[-2, -1]) for w in conv_w1])
...
def my_training(X_train, y_train, X_val, y_val, X_test, y_test, 
    ...
    print("Building my CNN. Parameters: ")
    ...
    with tf.name_scope('inputs'):
        xs = tf.placeholder(shape=[None, 256, 256, 3], dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        test_eve, merge_result = sess.run([eve, merge], feed_dict={xs: X_test, ys: y_test})
        test_acc = 100 - test_eve * 100 / y_test.shape[0]
        prediction = sess.run(pre[:,1]/(pre[:,0]+pre[:,1]), feed_dict={xs: X_test, ys: y_train[:3500]})
        print(test_acc)
