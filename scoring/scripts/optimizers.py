...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for t in grads:
            ...
            params[t] = params[t] - learning_rate * grads[t] / np.sqrt(cache[t] + eps)
        #raise NotImplementedError
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            #momentums[k] = momentums[k] / (1 - beta1 ** t)
            #velocitys[k] = velocitys[k] / (1 - beta2 ** t)
            #params[k] += - learning_rate * momentums[k] / (eps + np.sqrt(velocitys[k]))
            params[k] += - lr_t * momentums[k] / (np.sqrt(velocitys[k]) + eps)
        self.momentums = momentums
        self.velocitys = velocitys
        #raise NotImplementedError